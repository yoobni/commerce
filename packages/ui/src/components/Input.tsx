...
/**
 * Direction B Input spec:
 *   Border: 1px --mz-line → focus: 1.5px --mz-ink
 *   Radius: 10px  |  Padding: 14/16  |  Font: Inter 14/400
 *   floatingLabel: absolute top:-7px left:12px, bg page-bg,
 *                  Inter 600/10, +0.14em uppercase
 */
...
export const Input = forwardRef<HTMLInputElement, InputProps>(function Input(
  ...
  const baseInputClasses = cn(
    'w-full rounded-[var(--mz-radius-md,10px)]',
      ...
      <div className="flex flex-col gap-1.5 w-full">
        ...
        <div className="relative flex items-center">
          ...
          <input
            ...
            className={cn(
              baseInputClasses,
              'h-12 px-4 placeholder:text-[var(--mz-ink-mute)]',
              !!leadingIcon && 'pl-10',
    ...
    <div className="flex flex-col gap-1.5 w-full">
      <div className="relative flex items-center">
            ...
            className={cn(
              ...
              isFloated
                ? cn(
                    ...
                    'bg-[var(--mz-surface)]',
                    error ? 'text-[var(--mz-error,#c0392b)]' : 'text-[var(--mz-ink)]'
                  )
                : 'top-1/2 -translate-y-1/2 left-4 text-[14px] font-normal text-[var(--mz-ink-mute)]'
            )}
          >
            {label}
            {required && !isFloated && (
              <span className="text-[var(--mz-error,#c0392b)] ml-0.5" aria-hidden="true">*</span>
            )}
          </label>
        )}
        {leadingIcon && (
        ...
        <input
          ...
          className={cn(
            baseInputClasses,
            'h-14 px-4 placeholder-transparent',
            !!leadingIcon && 'pl-10',
            !!trailingAction && 'pr-10',
            className
          )}
