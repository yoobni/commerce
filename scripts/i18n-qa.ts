...
// Config
// ─────────────────────────────────────────
const MESSAGES_DIR = path.resolve(__dirname, '../apps/commerce/messages');
...
function report(results: QAResult[], reference: JsonObject): void {
  ...
  console.log(`\n${c(colors.bold, '═══════════════════════════════════════')}`);
  console.log(
    `${c(colors.bold, ' i18n QA Report')} ${c(colors.dim, `(reference: ${REFERENCE_LOCALE})`)}`
  );
  ...
  for (const r of results) {
    ...
    const status =
      issueCount === 0 ? c(colors.green, '✓ PASS') : c(colors.red, `✗ FAIL (${issueCount} issues)`);
    ...
    if (r.empty.length > 0) {
      console.log(
        `  ${c(colors.yellow, `Empty values (${r.empty.length}):`)} ${c(colors.dim, '(--strict)')}`
      );
  ...
  if (totalIssues === 0) {
    ...
    console.log(
      c(colors.dim, 'Run with --fix-hint to see reference (ko) values for missing keys.')
    );
  }
  console.log();
