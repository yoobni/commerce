...
export function LoginForm({ locale, next, hasOAuthError }: LoginFormProps) {
  ...
  // signIn 직후 redirect 만 client. next 는 이미 fully-qualified 라 그대로
  // location 에 박는다. router 가 한번 더 prefix 를 추가하는 wrapper 가
  // 없는 경로다.
  async function handleSubmit(e: FormEvent) {
    ...
    if (authError) {
      ...
      // Hard nav so server-side cookie is read for the next page and the
      // shell page can run its own user check / redirect logic if needed.
