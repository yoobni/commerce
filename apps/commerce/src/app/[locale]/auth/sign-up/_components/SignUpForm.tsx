...
interface SignUpFormProps {
  locale: Locale;
  /** Fully-qualified locale-prefixed destination (computed server-side). */
  next: string;
}

export function SignUpForm({ locale, next: _next }: SignUpFormProps) {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    if (authError) {
      ...
      // signUp → email confirmation 흐름. 자동 redirect 없이 안내 메시지만.
  ...
  async function handleGoogle() {
    setError(null);
    const { error: authError } = await signInWithGoogle(_next);
    if (authError) setError(t('error.generic'));
  }

  async function handleKakao() {
    setError(null);
    const { error: authError } = await signInWithKakao(_next);
    if (authError) setError(t('error.generic'));
  }

  async function handleNaver() {
    setError(null);
    const { error: authError } = await signInWithNaver(_next);
    if (authError) setError(t('error.generic'));
  }

  async function handleTwitter() {
    setError(null);
    const { error: authError } = await signInWithTwitter(_next);
    if (authError) setError(t('error.generic'));
  }
